| Метод              | URL                       | Опис                                            |
| ------------------ | ------------------------- | ----------------------------------------------- |
| HEAD, GET, OPTIONS | `/static/<path:filename>` | Доступ до статичних файлів (картинки, CSS, JS). |
...
| HEAD, GET, OPTIONS | `/api/users/<int:user_id>/stations` | Повертає всі станції, пов’язані з користувачем. |
...
| HEAD, GET, OPTIONS | `/api/stations/<int:station_id>/address` | Повертає адресу станції.                            |
| HEAD, GET, OPTIONS | `/api/stations/<int:station_id>/users`   | Повертає всіх користувачів, пов’язаних зі станцією. |
...
| HEAD, GET, OPTIONS | `/api/addresses/<int:address_id>/users` | Повертає всіх користувачів, які живуть за цією адресою (якщо прив’язка є). |
...
| HEAD, GET, OPTIONS | `/api/panels/<int:panel_id>/station` | Повертає станцію, до якої належить панель. |
| HEAD, GET, OPTIONS | `/api/panels/<int:panel_id>/type`    | Повертає тип панелі.                       |
...
| HEAD, GET, OPTIONS | `/api/batteries/<int:battery_id>/station` | Повертає станцію, до якої належить батарея. |
| HEAD, GET, OPTIONS | `/api/batteries/<int:battery_id>/charges` | Повертає всі заряди батареї.                |
...
| HEAD, GET, OPTIONS | `/api/panel-generations/<int:generation_id>/panel` | Повертає панель, до якої належить покоління. |
...
| HEAD, GET, OPTIONS | `/api/battery-charges/<int:charge_id>/battery` | Повертає батарею, до якої належить заряд. |
...
| HEAD, GET, OPTIONS | `/api/energy-sales/<int:sale_id>/station` | Повертає станцію, до якої належить продаж. |


| Метод              | URL                              | Опис                                                 |
| ------------------ | -------------------------------- | ---------------------------------------------------- |
| HEAD, GET, OPTIONS | `/api/<model_name>`              | Повертає всі записи будь-якої моделі (generic CRUD). |
| HEAD, GET, OPTIONS | `/api/<model_name>/<int:obj_id>` | Повертає один запис моделі.                          |
| POST, OPTIONS      | `/api/<model_name>`              | Створює запис будь-якої моделі.                      |
| PUT, OPTIONS       | `/api/<model_name>/<int:obj_id>` | Оновлює запис моделі.                                |
| DELETE, OPTIONS    | `/api/<model_name>/<int:obj_id>` | Видаляє запис моделі.                                |
